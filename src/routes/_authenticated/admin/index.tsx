    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">Live performance across your quizzes</p>
        </div>
      ...
      {data && (
        <>
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
            <Stat label="Quizzes" value={data.summary.quizzes} />
            <Stat label="Published" value={data.summary.published} />
          ...
          <Card>
            ...
            <CardContent>
              {data.trend.every((t) => t.attempts === 0)
                ? <p className="text-sm text-muted-foreground">No attempts yet.</p>
                : (
                  <div className="flex items-end gap-[2px] h-32">
                    {data.trend.map((t) => {
                      const max = Math.max(...data.trend.map((x) => x.attempts), 1);
