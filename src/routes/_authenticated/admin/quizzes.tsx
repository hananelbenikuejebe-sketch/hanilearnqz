...
import { Plus, Pencil, Trash2, Copy, BarChart3 } from "lucide-react";
          ...
          {(quizzes ?? []).map((q: any) => (
            <div key={q.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="icon" variant="ghost" asChild><Link to="/admin/quizzes/$id/edit" params={{ id: q.id }}><Pencil className="h-4 w-4" /></Link></Button>
                <Button size="icon" variant="ghost" asChild><Link to="/admin/quizzes/$id/results" params={{ id: q.id }}><BarChart3 className="h-4 w-4" /></Link></Button>
                <Button size="icon" variant="ghost" onClick={() => dup.mutate(q.id)}><Copy className="h-4 w-4" /></Button>
                <Button size="icon" variant="ghost" onClick={() => { if (confirm("Delete this quiz?")) del.mutate(q.id); }}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          ))}
